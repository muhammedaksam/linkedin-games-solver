...
function detectFinalSolveTime(): number | undefined {
  ...
  const clockIcon = document.querySelector('svg#clock-small, svg[data-test-icon="clock-small"], use[href="#clock-small"]');
  ...
  const timerSpan = parent.querySelector('span[role="text"], span[class*="timer"]');
...
async function saveGameCompleted(gameId: string, durationSeconds?: number) {
  ...
  try {
    ...
    const newTime = durationSeconds !== undefined && durationSeconds > 0
      ? durationSeconds
      : (existing?.time || 0);

    // Save if not yet logged, or if we have an active time reading to update a generic "0" reading
    if (!existing || (durationSeconds !== undefined && durationSeconds > 0 && existing.time === 0)) {
      ...
      console.log(`[LinkedIn Games Solver] Saved completion status for ${gameId}:`, history[dateKey][gameId]);
    }
  } catch (e) {
    console.error("[LinkedIn Games Solver] Failed to write completion state to storage:", e);
  }
}
...
async function checkVisitedGameSolved() {
  ...
  const seeResults = document.querySelector('a[href*="/results/"], a[href*="/results"], .games-share-footer');

  // 3. Check for standard results container/element data-testid (test IDs are hardcoded in source and never localized)
  const resultsPage = document.querySelector('[data-testid*="results"], [data-testid*="share"]');

  // 4. Check for locked board cell states (ARIA standard specs, identical in all languages)
  const cells = document.querySelectorAll('[data-testid^="cell-"], #tango-cell-0, [data-cell-idx]');
  const allCellsLocked = cells.length > 0 && Array.from(cells).every(cell => cell.getAttribute("aria-disabled") === "true");
  ...
  const underBoardControls = document.querySelector('[data-testid="under-board-controls"], .sudoku-under-board-controls-container');
  if (underBoardControls) {
    ...
    if (controlWrappers.length >= 2) {
      ...
      if (undoButton && hintButton && undoButton.hasAttribute('disabled') && hintButton.hasAttribute('disabled')) {
        controlsEnded = true;
      }
      
      // Mini Sudoku
      const sudokuHint = underBoardControls.querySelector('[data-control-btn="hint"]');
      const sudokuNotes = underBoardControls.querySelector('[data-control-btn="notes"]');
      if (sudokuHint && sudokuNotes && 
          (sudokuHint.classList.contains('sudoku-under-board__cta--disabled') || sudokuHint.getAttribute('aria-disabled') === 'true') && 
          (sudokuNotes.classList.contains('sudoku-under-board__cta--disabled') || sudokuNotes.getAttribute('aria-disabled') === 'true')) {
        controlsEnded = true;
      }
    }
  }
  ...
  const isSudokuEnded = !!document.querySelector('.games-share-footer') || 
                        !!document.querySelector('.grid-board--disabled') ||
                        (document.querySelectorAll('.sudoku-input-buttons__numbers button[disabled]').length === 6);

  const isGameEnded = isResultsUrl || 
                      !!seeResults || 
                      !!resultsPage || 
                      allCellsLocked || 
                      controlsEnded ||
...
function injectSolveButton() {
  ...
  if (underBoardControls.classList.contains('sudoku-under-board-controls-container') || underBoardControls.querySelector('[data-control-btn="hint"]')) {
    hintWrapper = underBoardControls.querySelector('[data-control-btn="hint"]') as HTMLElement;
  } else {
  ...
  try {
    ...
    let solveWrapper = underBoardControls.querySelector('.linkedin-solver-solve-btn-wrapper') as HTMLElement;
    if (!solveWrapper) {
      ...
      hintWrapper.parentNode?.insertBefore(solveWrapper, hintWrapper.nextSibling);
      console.log("[LinkedIn Games Solver] Injected Solve button next to Hint button successfully.");
    }
    ...
    let controlsEnded = false;
    const undoButton = controlWrappers[0]?.querySelector('button');
    const standardHint = controlWrappers[controlWrappers.length - 1]?.querySelector('button');
    if (undoButton && standardHint && undoButton.hasAttribute('disabled') && standardHint.hasAttribute('disabled')) {
      controlsEnded = true;
    }
    const sudokuHint = underBoardControls.querySelector('[data-control-btn="hint"]');
    const sudokuNotes = underBoardControls.querySelector('[data-control-btn="notes"]');
    ...
    if (isGameEnded) {
      ...
      solveButton.classList.add('is-disabled', 'sudoku-under-board__cta--disabled');
      ...
      solveButton.classList.remove('is-disabled', 'sudoku-under-board__cta--disabled');
    ...
    if (
      ...
      if (textTarget === solveButton && !solveButton.querySelector('.solve-btn-text-node')) {
    ...
    } else if (!textTarget.textContent || textTarget.textContent.trim() === '') {
      textTarget.textContent = 'Solve';
    }
    ...
    if (!solveButton.dataset.listenerBound) {
      ...
      solveButton.addEventListener('click', (e) => {
        ...
        if (!active) {
          console.warn("[LinkedIn Games Solver] No active game solver detected on this board.");
          return;
        }
        ...
        const labelNode = solveButton.querySelector('.solve-btn-text-node') || textTarget;
        ...
        active.solve()
          .then(() => {
            ...
            setTimeout(() => {
              ...
              const freshSeeResults = document.querySelector('a[href*="/results/"], a[href*="/results"], .games-share-footer');
              if (!freshSeeResults) {
                solveButton.removeAttribute('disabled');
                solveButton.setAttribute('aria-disabled', 'false');
                solveButton.classList.remove('is-disabled', 'sudoku-under-board__cta--disabled');
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.action === "solve") {
    ...
    active
      .solve()
      .then(() => {
        console.log(`[LinkedIn Games Solver] Solver ${active.name} completed successfully.`);
        const durationSeconds = Math.round((Date.now() - startTime) / 1000);
        saveGameCompleted(active.name.toLowerCase(), durationSeconds);
        sendResponse({ success: true });
      })
      .catch((err) => {
        console.error(`[LinkedIn Games Solver] Solver ${active.name} failed:`, err);
