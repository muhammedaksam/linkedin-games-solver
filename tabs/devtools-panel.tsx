...
export default function DevToolsPanel() {
  ...
  const getCellClassName = (cell: CellEntry) => {
    const base =
      "aspect-square rounded border border-border flex items-center justify-center font-bold text-[10px] sm:text-xs transition-all duration-200 select-none shadow-sm "
    if (cell.disabled) return base + "bg-muted/30 text-muted-foreground border-dashed"
    if (cell.text && cell.text !== "👑") return base + "bg-primary/10 text-primary border-primary/20"
    if (cell.text === "👑") return base + "bg-amber-500/10 text-amber-500 border-amber-500/30 animate-pulse"
  ...
  return (
    <div className="flex flex-col h-screen select-none overflow-hidden bg-background text-foreground font-sans">
      ...
      <header className="flex items-center justify-between px-4 border-b border-border bg-card h-12 shrink-0 shadow-sm">
        ...
        {tabUrl && (
          <div className="hidden md:flex items-center gap-1.5 text-[9px] font-medium text-muted-foreground max-w-[40%] truncate">
            <span className="shrink-0 font-bold uppercase">Url:</span>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-[10px] font-semibold text-muted-foreground">
            Inspected Tab: <span className="font-mono text-foreground font-bold">{chrome.devtools?.inspectedWindow?.tabId || "N/A"}</span>
          </span>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden min-h-0 bg-neutral-900/5">
        ...
        <div className="w-1/2 border-r border-border p-4 overflow-y-auto space-y-4 flex flex-col min-h-0">
          <div className="bg-card border border-border rounded-xl p-4 shadow-sm space-y-3.5 flex flex-col flex-1 min-h-0">
            <div className="flex items-center justify-between border-b border-border pb-2 shrink-0">
              <span className="font-bold text-xs text-foreground flex items-center gap-1.5">
                <Grid className="w-3.5 h-3.5 text-primary" />
                Live Matrix Grid Inspector
              </span>
              {gameName && (
            ...
            {gameName ? (
              <div className="flex-1 flex flex-col justify-center items-center overflow-auto min-h-0">
                {gridCells.length > 0 ? (
                  <div
                    className="grid gap-1 p-1 max-w-[280px] w-full"
                    style={{
                      gridTemplateColumns: `repeat(${Math.ceil(Math.sqrt(gridCells.length))}, minmax(0, 1fr))`
                    }}>
                    {gridCells.map((cell) => (
                      <div
                        key={cell.id}
                  ...
                  <div className="text-[10px] text-muted-foreground italic text-center py-8">
                    Parsing game grid cells... Ensure the board has active grid blocks visible.
                  </div>
                )}
              </div>
            ) : (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-6 min-h-0">
                ...
                <h4 className="text-xs font-bold text-foreground">Waiting for LinkedIn Game Page</h4>
                <p className="text-[10px] text-muted-foreground max-w-[220px] leading-relaxed mt-1">
                  Navigate to any LinkedIn Game board tab (Queens, Sudoku, Tango, Pinpoint) to capture constraint coordinates, cell states, and reactive structures.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="w-1/2 p-4 overflow-y-auto flex flex-col min-h-0">
          <div className="bg-card border border-border rounded-xl p-4 shadow-sm space-y-3.5 flex flex-col flex-1 min-h-0">
            {/* Logs Toolbar */}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className="flex items-center justify-center h-6 w-6 rounded hover:bg-muted text-muted-foreground hover:text-foreground border border-transparent hover:border-border disabled:opacity-40"
                    title="Copy Logs">
                  ...
                  <button
                    ...
                    className="flex items-center justify-center h-6 w-6 rounded hover:bg-muted text-muted-foreground hover:text-destructive border border-transparent hover:border-border"
                    title="Clear Log Space">
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full text-[10px] pl-7 pr-2.5 h-7 rounded border border-border bg-muted/20 hover:border-primary/20 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 text-foreground"
                  />
                </div>
                ...
                <select
                  ...
                  className="text-[10px] px-1.5 h-7 rounded border border-border bg-muted/20 focus:outline-none text-foreground">
            ...
            <div className="flex-1 overflow-hidden min-h-0 relative">
              {filteredLogs.length > 0 ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-center p-6 border border-dashed border-border rounded-lg bg-muted/5 min-h-0">
                  ...
                  <h4 className="text-xs font-bold text-foreground">Console Stream Empty</h4>
                  <p className="text-[10px] text-muted-foreground max-w-[200px] leading-relaxed mt-1">
                    No log outputs detected matching filters. Run steps inside LinkedIn Games solver UI to stream log actions here.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-1.5 justify-center text-[8px] font-semibold text-muted-foreground pt-1.5 select-none shrink-0 border-t border-border">
              ...
              Subscribed to storage.session stream
            </div>
          </div>
        </div>
      </div>
