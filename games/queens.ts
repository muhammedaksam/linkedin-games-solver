...
export class QueensSolver extends BaseSolver {
  ...
  async solve(mode: "full" | "hint" = "full"): Promise<void> {
    ...
    console.log(
      "[Queens] Solution coordinates:",
      Array.from(solution).sort((a, b) => a - b)
    )
    ...
    if (mode === "hint") {
      ...
      for (const idx of solution) {
        if (!userQueens.has(idx)) {
          console.log(
            `[Queens] Placing a single hint Queen at coordinate: ${idx}`
          )
  ...
  private inferN(): number {
    ...
    if (!grid) {
      throw new Error(
        "Could not find the Queens grid: [data-testid='interactive-grid']"
      )
    }
  ...
  private buildPuzzle(N: number): {
    ...
    for (let idx = 0; idx < N * N; idx++) {
      ...
      if (
        this.readCellState(el) === 1 ||
        (this.isLocked(el) && this.readCellState(el) === 1)
      ) {
        givenQueens.add(idx)
      }
    }
