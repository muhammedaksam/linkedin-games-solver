...
export class TangoSolver extends BaseSolver {
  ...
  async solve(): Promise<void> {
    const N = this.inferN();
    if (N % 2 !== 0) {
      throw new Error(`Tango expects an even board dimension N. Detected N: ${N}`);
    }
  ...
  private inferN(): number {
    ...
    if (guess * guess !== total) {
      throw new Error(`Failed to infer Tango grid size from total cells: ${total}`);
    }
    return guess;
  }
  ...
  private parseEdges(N: number): EdgeConstraint[] {
    ...
    for (let idx = 0; idx < N * N; idx++) {
      ...
      const eqSvgs = this.$$('svg[data-testid="edge-equal"]', cell).map((s) => ({
        svg: s,
        type: "eq" as const,
      }));
      const neqSvgs = this.$$('svg[data-testid="edge-cross"]', cell).map((s) => ({
        svg: s,
        type: "neq" as const,
      }));
  ...
  private buildAdjMap(N: number, constraints: EdgeConstraint[]): AdjConstraint[][] {
  ...
  private isConsistent(g: number[][], N: number, adj: AdjConstraint[][]): boolean {
  ...
  private propagate(g: number[][], N: number, adj: AdjConstraint[][]): number[][] {
    let changed = true;
    ...
    const applyTriples = (arr: number[], writeIdx: (i: number, val: number) => boolean): boolean => {
      ...
      for (let i = 0; i < N; i++) {
        if (i + 2 < N) {
          const a = arr[i],
            ...
            c = arr[i + 2];
          // XX_ -> O
          if (a !== -1 && a === b && c === -1) localChanged = writeIdx(i + 2, 1 - a) || localChanged;
          // _XX -> O
          if (a === -1 && b !== -1 && b === c) localChanged = writeIdx(i, 1 - b) || localChanged;
          // X_X -> O
          if (a !== -1 && c !== -1 && a === c && b === -1) localChanged = writeIdx(i + 1, 1 - a) || localChanged;
        }
      }
      return localChanged;
    };
  ...
  private solveTango(g0: number[][], N: number, adj: AdjConstraint[][]): number[][] | null {
  ...
  private async setCellUI(idx: number, targetVal: number): Promise<void> {
    ...
    console.warn(`[Tango] Could not cycle cell ${idx} to target value: ${targetVal}`);
  }
}
