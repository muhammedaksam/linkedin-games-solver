...
export class SudokuSolver extends BaseSolver {
      ...
      (this.$$("[data-cell-idx]").length > 0 &&
        (!!this.$(".sudoku-cell") ||
          this.$$("[data-cell-idx]").some((cell) =>
            cell.className.includes("sudoku-cell")
          )))
    )
  }
  ...
  async solve(mode: "full" | "hint" = "full"): Promise<void> {
    ...
    console.log(
      `[Sudoku] Detected ${N}x${N} board. Region method: ${regionInfo.kind}`
    )
    ...
    if (!ok) {
      ...
      throw new Error(
        "No Sudoku solution found! (Could be a parsing mismatch or region inference error.)"
      )
    }
    ...
    if (mode === "hint") {
      ...
      const userPlaced = Array.from({ length: N }, () =>
      ...
      if (errorCells.length > 0) {
        for (const cell of errorCells) {
          cell.style.transition =
            "background-color 200ms, outline 200ms, border-color 200ms"
        ...
        throw new Error(
          "Mistake detected! Correct the highlighted cell(s) first."
        )
      }
      ...
      for (let idx = 0; idx < N * N; idx++) {
        ...
        if (givenGrid[r][c] === 0 && userPlaced[r][c] === 0) {
          ...
          if (val) {
            console.log(
              `[Sudoku] Placing a single hint digit ${val} at cell index: ${idx}`
            )
  ...
  private hasWallBetween(
    ...
    if (direction === "R") {
      return (
        aCls.contains("sudoku-cell-wall-right") ||
        bCls.contains("sudoku-cell-wall-left")
      )
    }
    if (direction === "L") {
      return (
        aCls.contains("sudoku-cell-wall-left") ||
        bCls.contains("sudoku-cell-wall-right")
      )
    }
    if (direction === "D") {
      return (
        aCls.contains("sudoku-cell-wall-bottom") ||
        bCls.contains("sudoku-cell-wall-top")
      )
    }
    if (direction === "U") {
      return (
        aCls.contains("sudoku-cell-wall-top") ||
        bCls.contains("sudoku-cell-wall-bottom")
      )
    }
    return true
  }
  ...
  private getNumberButton(v: number): HTMLButtonElement | null {
    return Array.from(document.querySelectorAll("button")).find((b) => {
      return (
        b.textContent?.trim() === String(v) && !b.hasAttribute("data-cell-idx")
      )
