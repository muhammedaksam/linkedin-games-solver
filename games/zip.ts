...
export class ZipSolver extends BaseSolver {
    ...
    for (const c of cells) {
      ...
      for (let i = 1; i < children.length; i++) {
        ...
        const hasClass = (el: HTMLElement, cls: string) =>
          el.classList.contains(cls) || Array.from(el.classList).some((cname) => cname.includes(cls));
          ...
          if (width > 0 && height > 0) {
            ...
            if (width > height) {
              ...
              if (wRect && wRect.width > 0 && wRect.height > 0) {
                ...
                if ((top === "0px" || parseFloat(top) < 10) && r > 0) neighbor = idx - N;
                else if (r < N - 1) neighbor = idx + N;
              }
              ...
              if (wRect && wRect.width > 0 && wRect.height > 0) {
                ...
                if ((left === "0px" || parseFloat(left) < 10) && col > 0) neighbor = idx - 1;
                else if (col < N - 1) neighbor = idx + 1;
              }
            }
          }
  ...
  private solveZip(N: number, checkpoints: Map<number, number>, walls: Set<string>): number[] | null {
    const total = N * N;
